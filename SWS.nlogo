...
people-own [ zone sex age qhealth car ed NSSEC numdu98 costdly tpaste costTyp psycdisc everadsm highsug regular fluoride sweets ipout ncakes clnthg3 freqden evrfrqy rteacc totohip crime_score year_lost inc_est h_price
  count-down to-node speed cur-link target destination destination-entrance mynode step-in-path mypath last-stop birth-ticks ticktime]
globals [ csv filelist sheffield-west-dataset sheffield-west-buildings sheffield-west-roads sheffield-west-dentists sheffield-west-shops got_to_shop got_to_FE got_to_dentist_D got_to_shop_D got_to_FE_D got_to_dentist_O
  got_to_shop_O got_to_FE_O schedule pathway3-count]
patches-own [ centroid? id entrance name CODE crime years_lost income hprice is-road? centroid?2 ]
...
to setup
  ...
  set sheffield-west-dataset gis:load-dataset "Sheffield_west_data/Sheffield_west_buffer.shp"
  set sheffield-west-buildings gis:load-dataset "Sheffield_west_data/Sheffield_west_buildings_buffer.shp"
  set sheffield-west-roads gis:load-dataset "Sheffield_west_data/Sheffield_west_roads_buffer.shp"
  set sheffield-west-dentists gis:load-dataset "Sheffield_west_data/Sheffield_west_dentists.shp"
  ;set sheffield-west-shops gis:load-dataset "Sheffield_west_data/Sheffield_west_dentists.shp"

  set sheffield-west-dentists gis:load-dataset "Sheffield_west_data/Sheffield_west_buildings_dentists.shp"
  ;set sheffield-west-dentists gis:load-dataset "Sheffield_west_data/Sheffield_west_buildings_dentists_extra.shp"
  set sheffield-west-shops gis:load-dataset "Sheffield_west_data/Sheffield_west_shops_extra.shp"

  let road-envelope gis:envelope-of sheffield-west-roads
  ...
  gis:set-drawing-color yellow gis:fill sheffield-west-dentists 1
  gis:set-drawing-color grey gis:draw sheffield-west-buildings 1
  gis:set-drawing-color [  0   0 255]    gis:draw sheffield-west-roads 1
  gis:set-drawing-color pink gis:fill sheffield-west-shops 1

  ask patches [set pcolor white]
  create-the-people

     foreach gis:feature-list-of sheffield-west-dentists
  [ let center-point gis:location-of gis:centroid-of ?
   ...
   foreach gis:feature-list-of sheffield-west-shops
  [ let center-point gis:location-of gis:centroid-of ?
    ask patch item 0 center-point item 1 center-point [
      set centroid?2 true
      set id gis:property-value ? "id"
...
to move-dentist-degree
    ...
    if destination = nobody [
       ...
       while [destination-entrance = mynode] [set destination one-of patches with [centroid?2 = true] set destination-entrance [entrance] of destination ]
...
to d2p1
    if ticks mod 7 = 0[
    ifelse h_price <= 251636 and costdly = 1
...
to d3p1
  if ticks mod 14 = 0[
    ifelse nssec = 8 and inc_est <= 1056.1  [ ; gone for this option for income rather than 'influence-of-income' slider - the slider option doesn't consider that actual value of the income variable, and applies it equally regardless of income level...
      set numdu98 numdu98 + 0.01]
    [set numdu98 numdu98 - 0.01]
  ]
end
...
to d4p3
  if ticks mod 14 = 0[
    ifelse year_lost > 52.94 and freqden > 2
    [set fluoride fluoride + 0.01]
    [set fluoride fluoride - 0.01]
  ]
  d4p3-tbrush
...
to d4p5
  if ticks mod 7 = 0[
    ifelse crime_score > -1 and regular > 2
    [set numdu98 numdu98 + 0.01]
    [set numdu98 numdu98 - 0.01]
  ]
end
...
to create-the-people
  ...
  create-people 8644 [ set color blue set shape "person" set size 0.2 set count-down 15 set birth-ticks ticks ] ; 8644 for 'west_sheffield_cluster.csv'

  ;file-open "Sheffield_west_data/west_sheffield_cluster_nofactor_sample.csv" ; for the sample with numbers rather than factors
  file-open "Sheffield_west_data/sheffield_west_cluster_NOfactor_master.csv"
...
to setup-paths-graph
  ...
  foreach polylines-of sheffield-west-roads node-precision [
...
foreach gis:feature-list-of sheffield-west-roads[
...
ask patches [set pcolor white]
;  ask patches gis:intersecting sheffield-west-dentists
;  [set pcolor yellow]
;  ask patches gis:intersecting sheffield-west-shops
;  [set pcolor pink]
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot sum [numdu98] of people"
...
635
NIL
100 - (sum [numdu98] of people / 8644 * 100)
2
1
11
